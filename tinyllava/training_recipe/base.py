...
class BaseTrainingRecipe:
    ...
    def _llm_tune_type_setting(self, model):
        ...
        assert tune_type in ('frozen', 'full', 'lora', 'qlora', 'part'), f'tune_type {tune_type} not supported in this training recipe!'
        if tune_type == 'full':
    ...
    def load(self, model, model_args={}):
        if self.training_arguments.resume_from_checkpoint and not 'lora' in self.training_arguments.resume_from_checkpoint:
            model = model.from_pretrained(self.training_arguments.resume_from_checkpoint)
        elif not ('lora' in self.training_arguments.pretrained_model_path and os.path.exists(os.path.join(self.training_arguments.pretrained_model_path, 'adapter_config.json'))): # loading model for non-lora/non-qlora pretraining
